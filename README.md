...
Thus, $B = \theta \hat{B}$, where $\theta = \lVert u_\perp \rVert / \sqrt{d}$ and $\hat{B} = \hat{x}\,\hat{u}_{\perp}^{\mathsf T} - \hat{u}_{\perp}\,\hat{x}^{\mathsf T}$ is constructed from the pair of orthogonal unit vectors $(\hat{x}, \hat{u}_\perp)$ as above, with $\hat{x} = x / \sqrt{d}$. Equations (5) and (8) retain their form with the redefined $\theta$.
